...
convert_system_info(dist_ctrl, List) ->
    [{Node, pid_to_binary(Ctrl)} || {Node, Ctrl} <- List];
convert_system_info(driver_version,     Value) -> list_to_binary(Value);
convert_system_info(machine,            Value) -> list_to_binary(Value);
convert_system_info(otp_release,        Value) -> list_to_binary(Value);
convert_system_info(scheduler_bindings, Value) -> tuple_to_list(Value);
convert_system_info(system_version,     Value) -> list_to_binary(Value);
convert_system_info(system_architecture,Value) -> list_to_binary(Value);
convert_system_info(version,            Value) -> list_to_binary(Value);
convert_system_info(_,                  Value) -> Value.

convert_allocated_areas({Key, Value1, Value2}) -> {Key, [Value1, Value2]};
convert_allocated_areas({Key, Value}) -> {Key, Value}.
...
pid_to_binary(PidOrPort) -> list_to_binary(io_lib:format("~p", [PidOrPort])).
